          ...
          <div className="flex flex-col justify-between">
            {props.link ? (
              <Link href={props.link} target="_blank" rel="noreferrer">
                <Text as="h5">
                  {props.title} <FontAwesomeIcon icon={faExternalLink} />
                </Text>
              </Link>
            ) : (
    ...
    <div className="my-4 gap-4 flex flex-col md:flex-row">
      {props.logo && (
        <Frame
          ...
          size="medium"
        >
          <Image alt="Personal" src={props.logo} width={1000} height={1000} />
        </Frame>
      )}

      <Frame
        className={`px-8 py-4 basis-full ${props.logo && "md:basis-2/3"}`}
        illuminator={500}
        type="kranox"
        size="small"
      >
